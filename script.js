...
// Lógica MT paso a paso
function step() {
    if (tape.length === 0) return;

    const symbol = tape[head];
    const estadoAnterior = currentState;

    switch (currentState) {
        case "q0":
            if (symbol === "I") {
                logProceso(estadoAnterior, symbol, "Reconoce inicio 'I'", "Derecha", "q1",
                           "Inicio del prefijo INV");
                head++;
                currentState = "q1";
            } else reject("Esperaba 'I'");
            break;

        case "q1":
            if (symbol === "N") {
                logProceso(estadoAnterior, symbol, "Reconoce 'N'", "Derecha", "q2",
                           "Segundo símbolo del prefijo INV");
                head++;
                currentState = "q2";
            } else reject("Esperaba 'N'");
            break;

        case "q2":
            if (symbol === "V") {
                logProceso(estadoAnterior, symbol, "Reconoce 'V'", "Derecha", "q3",
                           "Completa el prefijo INV");
                head++;
                currentState = "q3";
            } else reject("Esperaba 'V'");
            break;

        case "q3":
            if (symbol === "-") {
                logProceso(estadoAnterior, symbol, "Reconoce '-'", "Derecha", "qDigits",
                           "Separador antes de los dígitos");
                head++;
                currentState = "qDigits";
            } else reject("Esperaba '-'");
            break;

        case "qDigits":
            if (/[0-9]/.test(symbol)) {
                logProceso(estadoAnterior, symbol, "Reconoce dígito", "Derecha", "qDigits",
                           "Parte numérica del código");

                head++;
                if (head >= tape.length) accept();

            } else reject("Esperaba dígito");
            break;
    }
...
function accept() {
    ...
    stateLabel.innerHTML = `
        <div class='log-item'><strong>Cadena aceptada</strong> → El código cumple la gramática INV-[0-9]+</div>
    `;
...
function reject(reason) {
    ...
    stateLabel.innerHTML = `
        <div class='log-item'><strong>Error:</strong> ${reason} → No cumple la gramática INV-[0-9]+</div>
    `;
